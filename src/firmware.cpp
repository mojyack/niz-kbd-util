...
auto flush_firmware(int fd, const char* const firmware_path) -> bool {
    ...
    std::println("sending firmware");
    for(auto i = 0uz; i < firmware.size(); i += 1) {
        ...
        std::println("{}/{} {} bytes", i + 1, firmware.size(), part.size());
